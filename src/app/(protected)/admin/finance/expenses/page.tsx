...
import { getMembersAndEvents, registerExpense } from "@/server/actions/finance";
...
export default function ExpensesPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setMembers(members);
            setEvents(events);
            // Default select all members? Or none? User image shows "Valgt: 5", implying manual selection.
            // Let's default to selecting all for convenience, or none. 
            // Better to select none to force conscious decision, or all if that's the common case.
            // Let's start with none to be safe.
            setLoading(false);
        };
    ...
    if (dashboardData?.member.role !== "ADMIN") return null;
        ...
        <div className="w-full space-y-4 pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col h-full overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto max-h-[400px] p-2 space-y-0.5">
                    ...
                    <div className="p-4 bg-gray-900 text-white mt-auto">
                        <div className="flex justify-between items-center mb-1 text-xs text-gray-400">
                            ...
                            <span className="font-medium text-white">{new Intl.NumberFormat("nb-NO", { style: "currency", currency: "NOK" }).format(totalAmount)}</span>
                        </div>
                        <div className="flex justify-between items-center mb-3 text-xs text-gray-400">
                            <span>Fordelt på</span>
                            <span className="font-medium text-white">{splitCount} medlemmer</span>
                        </div>
                        <div className="pt-3 border-t border-gray-700">
                            <div className="flex justify-between items-end mb-3">
                                <span className="text-xs font-bold text-indigo-400 uppercase tracking-widest">TOTAL PER PERS</span>
                                <span className="text-2xl font-bold">{new Intl.NumberFormat("nb-NO", { style: "currency", currency: "NOK", maximumFractionDigits: 0 }).format(amountPerPerson)}</span>
                            </div>
                            <button
                                onClick={handleSubmit}
                                disabled={submitting || !amount || selectedMemberIds.length === 0}
                                className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-lg font-bold text-sm text-white transition-all shadow-lg hover:shadow-indigo-500/25 active:scale-95"
                            >
